...
def fetch_all_stocks(days: int = 5) -> Dict[str, List[Dict[str, Any]]]:
    ...
    print(f"[MARKET MAP] Fetching {len(all_tickers)} stocks via batch download...")
    ...
    try:
        ...
        for sector, tickers in SP500_SECTORS.items():
            ...
            if success_rate == 100:
                print(f"[OK] {sector}: {len(sector_stocks)}/{len(tickers)}")
            elif success_rate >= 50:
                print(f"[WARN] {sector}: {len(sector_stocks)}/{len(tickers)} ({success_rate:.0f}%)")
            else:
                print(f"[ERROR] {sector}: {len(sector_stocks)}/{len(tickers)} ({success_rate:.0f}%)")
        
        total = sum(len(s) for s in sector_results.values())
        print(f"\n[MARKET MAP] Total: {total}/{len(all_tickers)} stocks ({100*total/len(all_tickers):.1f}%)\n")
...
async def get_market_map_data(days: int = 5):
    ...
    if _cache["data"] and _cache["timestamp"]:
        ...
        if age < CACHE_TTL:
            print(f"[CACHE] Market data cache hit (age: {int(age)}s)")
            return _cache["data"]
    
    # Fetch fresh data
    sector_results = fetch_all_stocks(days)
    
    # Fetch SPY week performance for daily summary cards
    week_performance = []
    try:
        spy = yf.Ticker("SPY")
        spy_hist = spy.history(period="10d")  # Get extra days to calculate day-over-day changes
        if not spy_hist.empty and len(spy_hist) > 1:
            # Calculate day-over-day percentage changes
            spy_hist['pct_change'] = spy_hist['Close'].pct_change() * 100
            # Take the last 5 days
            for idx, (date, row) in enumerate(spy_hist.tail(5).iterrows()):
                ...
                elif idx == 0:  # First day doesn't have a previous close, use intraday
                    open_price = row['Open'] if 'Open' in row and pd.notna(row['Open']) else row['Close']
                    pct_change = ((row['Close'] - open_price) / open_price) * 100 if open_price > 0 else 0
                    week_performance.append({
                        "date": date.strftime("%Y-%m-%d"),
                        "day_name": date.strftime("%a"),
        ...
        print(f"[WARN] Failed to fetch SPY week performance: {e}")
...
async def get_spy_intraday():
    ...
    for index_name, ticker_symbol in indices.items():
        try:
            ...
            data = ticker.history(start=start_date, end=end_date, interval="5m", timeout=30, prepost=False)
            ...
            print(f"[OK] {index_name} intraday: {len(unique_dates)} days")
        except Exception as e:
            print(f"[ERROR] {index_name} intraday failed: {e}")
